...
class BlogGenerator:
    def __init__(self, blog_dir="/Users/sarveshbhatnagar/Development/profile/blog"):
    ...
    def generate_html(self, markdown_file):
        ...
        description = metadata.get(
            "description", f"A blog post by Sarvesh Bhatnagar about {title}"
        )
    ...
    def generate_blog_list(self):
        ...
        for html_file in self.blog_dir.glob("*.html"):
            if html_file.name not in ["mainPage.html"]:
                ...
                blog_files.append(
                    {"filename": html_file.name, "title": title, "url": html_file.name}
                )

        return blog_files

    def regenerate_posts_data_js(self):
        """Regenerate posts/posts-data.js from posts/posts.json for file:// support"""
        posts_json = self.blog_dir / "posts" / "posts.json"
        posts_data_js = self.blog_dir / "posts" / "posts-data.js"
        if not posts_json.exists():
            return
        with open(posts_json, "r", encoding="utf-8") as f:
            data = f.read().strip()
        with open(posts_data_js, "w", encoding="utf-8") as f:
            f.write(f"window.BLOG_POSTS = {data};\n")
        print(f"Regenerated: {posts_data_js}")
    ...
    def sync_posts_json(self, markdown_file):
        """Upsert a posts.json entry from a markdown file"""
        ...
        try:
            iso_date = datetime.strptime(str(raw_date), "%B %d, %Y").strftime("%Y-%m-%d")
        except ValueError:
        ...
        posts = [p for p in posts if p["id"] != slug]
        posts.insert(0, entry)

        with open(posts_json, "w", encoding="utf-8") as f:
            json.dump(posts, f, indent=2)

        print(f"Synced posts.json: {slug}")
    ...
    def build_single(self, markdown_file):
        ...
        self.sync_posts_json(markdown_file)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate HTML blog posts from Markdown"
    )
    parser.add_argument(
        "--build-all", action="store_true", help="Build all markdown files"
    )
    parser.add_argument("--file", help="Build specific markdown file")
    parser.add_argument(
        "--blog-dir",
        default="/Users/sarveshbhatnagar/Development/profile/blog",
    ...
    if args.build_all:
        ...
        # Update main page and data files
        blog_files = generator.generate_blog_list()
        generator.update_main_page(blog_files)
        generator.regenerate_posts_data_js()
        print("Done.")
    else:
        print(
            "Use --build-all to build all markdown files or --file <path> to build a specific file"
        )
